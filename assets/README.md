...
[shabirbim.com](https://shabirbim.com) · [engrshabir@shabirbim.com](mailto:engrshabir@shabirbim.com) · [linkedin.com/in/shabir-bim](https://linkedin.com/in/shabir-bim)
...
**2 — Classification**  
Multi-layer detection using PredefinedType, element naming conventions, material keywords, authoring software-specific signals (Gewerk for Allplan, BaseQuantities absence for ArchiCAD library objects, BASESLAB for Revit footings). Software detected from IFC header at load time.
...
These are real extraction failures caught across the seven files. Each one is documented with exact numbers in the case studies.

**Non-dimensional property as geometry input (File 01)**  
`Pset\_BeamCommon.Slope = 1718.873` on 42 timber rafters. Read naively as a dimensional input, this produces a beam depth of 37.65 m. Actual rafter depth: 0.16 m. The non-dimensional property blocklist — Slope, FireRating, ThermalTransmittance, LoadBearing — prevents this class of error permanently.
...
Cost figures in all reports use default demonstration rates (€120/m³ concrete, €1.2/kg steel). These are indicative only — not market rates.
